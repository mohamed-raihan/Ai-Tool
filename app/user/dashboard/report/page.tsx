...
type Category = 'Introvert' | 'Analytical' | 'Realistic' | 'Practical' | 
...
export const calculateScores = (responses: Record<string, string>, questions: Array<{ id: string, category: string }>): TestResult => {
  ...
  const scores = Array.from(allCategories).map(category => {
    ...
    const score = data.total > 0 ? Math.round((data.score / data.total) * 100) : 0;
    totalScore += score;
  ...
  const overallScore = scores.length > 0 ? Math.round(totalScore / scores.length) : 0;
...
function getOverallInterpretation(scores: Array<{ category: string; score: number }>): string {
  const topScores = scores
    .sort((a, b) => b.score - a.score)
    .slice(0, 3);
  
  return `Your assessment shows strong tendencies in ${topScores.map(s => s.category).join(', ')}. 
